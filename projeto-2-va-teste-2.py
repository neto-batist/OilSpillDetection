...
class OilSpillDataset(Dataset):
    ...
    def parse_annotation(self, label_path):
        if not os.path.exists(label_path):
            raise ValueError(f"O arquivo de anotação {label_path} não foi encontrado.")
...
def collate_fn(batch):
    ...
    if not batch:
        return torch.tensor([]), []  # Retornar um tensor vazio e uma lista vazia
...
for epoch in range(NUM_EPOCHS):
    ...
    for images, targets in train_loader:
        if len(images) == 0:  # Verificar se o batch está vazio
            print(f"Batch vazio encontrado na época {epoch + 1}. Pulando...")
            continue
